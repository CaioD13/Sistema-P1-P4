...
def _gerar_escala_semanal(cia, inicio, tipo_filtro='TODOS'):
    try:
        ...
        ws.merge_range('C1:Q1', "SECRETARIA DA SEGURANÇA PÚBLICA", fmt_cabecalho)
        ws.merge_range('C2:Q2', "POLÍCIA MILITAR DO ESTADO DE SÃO PAULO", fmt_cabecalho)
        ws.merge_range('C3:Q3', "COMANDO DE POLICIAMENTO DE ÁREA METROPOLITANA DEZ", fmt_cabecalho)
        ws.merge_range('C4:Q4', "TRIGÉSIMO SÉTIMO BATALHÃO DE POLICIA MILITAR METROPOLITANO", fmt_cabecalho)

        texto_escala = f"ESCALA DE SERVIÇO DE {tipo_txt} Nº 37BPMM-{numerador} - {versao_txt}"
        ws.merge_range('A6:Q6', texto_escala, fmt_numerador)
        ws.merge_range('A7:Q7', periodo_txt, fmt_cabecalho)

        linha = 8
        cols = ["EQ", "POSTO", "RE", "NOME", "VTR", "ENT", "SAI", "FUNÇÃO", "SEÇÃO", "STATUS"]
        ...
        for i in range(7):
            d = inicio + timedelta(days=i)
            ws.write(linha, 10 + i, f"{dias_semana[i]}\n{d.day}/{d.month}", fmt_dia)
        ...
        for nome_bloco in ordem:
            ...
            ws.merge_range(f'A{linha}:Q{linha}', f'{nome_bloco}', fmt_bloco)
            linha += 1
            ...
            for pm in lista_pms:
                ...
                ws.write(linha, 9, status, fmt_center)

                for i in range(7):
                    data_dia = inicio + timedelta(days=i)
                    info_dia = calcular_escala_dia(pm, data_dia)
                    sigla_dia = (info_dia.get('sigla') or '').strip().upper()
                    descricao_dia = info_dia.get('descricao', sigla_dia)
                    ...
                    if sigla_dia == 'TRABALHO':
                        ...
                        vis = descricao_dia if descricao_dia else sigla_dia
                        estilo = fmt_afastamento if is_afastamento(sigla_dia) else fmt_center

                    ws.write(linha, 10 + i, vis, estilo)
