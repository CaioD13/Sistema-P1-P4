...
def traduzir_para_frequencia(sigla):
    ...
    if sigla in ['S', 'S/1', 'S/2', 'ADM', 'EXP', '8H', '9H', '10H', 'EAP']: 
        return '1'
...
def salvar_pm():
    try:
        ...
        # O checkbox retorna 'on' (True) se marcado, ou None (False) se desmarcado
        ...
        # Tratamento da data de fim de restrição
        ...
        # Verifica se estamos editando ou criando um novo
        if id_pm:
            ...
            pm = Policial()
            db.session.add(pm)
            pm.ativo = True # Garante que o novo PM nasça ativo
            mensagem = 'Novo policial cadastrado com sucesso!'
        
        # Atualizando as informações no objeto do banco
        pm.re = re
        pm.posto = posto
        pm.nome = nome
        ...
        # === NOVA LÓGICA DE ÂNCORA DE MÊS (ESCALA OPERACIONAL) ===
        # Salva o dia 1 ou 2 do mês/ano exato em que a alteração está sendo feita
        hoje = datetime.now().date()
        
        if inicio_pares_req:
            # Fixa no dia 2 do mês corrente
            pm.data_base_escala = date(hoje.year, hoje.month, 2)
        else:
            # Fixa no dia 1 do mês corrente
...
def salvar_alteracao():
    try:
        ...
        # 0. Verifica se o PM é do Administrativo
        ...
        # Validação para Administrativos (JM/M1/M2 para edições de 1 dia de cada vez)
        if e_administrativo and valor in ['JM', 'M1', 'M2'] and (fim - inicio).days == 0:
            
            # Encontrar a Segunda-feira da semana atual
            dia_da_semana = inicio.weekday() # 0 = Segunda, 6 = Domingo
            ...
            # Lista COMPLETA de folgas/afastamentos (agora incluindo 'F', 'LP', etc.)
            todas_folgas = ['F', 'A', 'JM', 'FM', 'DS', 'DR', 'FR', 'PF', 'CV', 'LTS', 'EAP', 'LT', 'LG', 'PT', 'LA', 'NP', 'FN', 'FS', 'LP', 'LSV', 'AD']
            
            # Varre os 5 DIAS ÚTEIS da semana (Segunda a Sexta)
            for d in range(5): 
                dia_analise = segunda + timedelta(days=d)
                
                if dia_analise != inicio:
                    # Analisa os OUTROS 4 dias úteis da semana (o que já está no banco ou na escala padrão)
                    sigla = calcular_escala_dia(pm, dia_analise)
                    
                    if sigla in todas_folgas:
                    ...
                    if traduzir_para_frequencia(sigla) in ['1', '2', '3']:
                        contagem_trabalho += 1
                else:
                    # No dia em que está sendo feita a alteração, simula o valor que o usuário quer salvar
                    if traduzir_para_frequencia(valor) in ['1', '2', '3']:
                        contagem_trabalho += 1
            ...
            if valor == 'JM':
                ...
                # SE JÁ EXISTEM 2 OU MAIS FOLGAS NOS OUTROS DIAS DA SEMANA, BLOQUEIA O JM!
                if contagem_folgas_outros_dias >= 2: 
                    flash('⚠️ Bloqueado: O PM já possui 2 ou mais dias de folga/afastamento na semana (sem contar o FDS). Junção de Meios não permitida.', 'danger')
                    return redirect(request.referrer)

            elif valor in ['M1', 'M2']:
                if contagem_trabalho < 2:
                    flash('⚠️ Bloqueado: Para fazer jus a Meio Expediente (M1/M2), o PM deve trabalhar pelo menos 2 dias inteiros na semana.', 'danger')
                    return redirect(request.referrer)

        # --- FIM DAS VALIDAÇÕES ---

        # Salva no banco de dados se passar por todas as regras
