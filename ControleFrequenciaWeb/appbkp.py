...
def exportar_excel():
    try:
        ...
        # Configuração de Impressão (Paisagem, Ajustado para 1 página de largura)
        ws.set_paper(9) # A4
        ws.set_landscape()
        ws.set_margins(0.3, 0.3, 0.3, 0.3)
        ws.fit_to_pages(1, 0)
        
        # --- ESTILOS VISUAIS (Bordas Finas, Fonte Arial) ---
        ...
        fmt_header = workbook.add_format({
            ...
            'align': 'center', 'valign': 'vcenter', 'border': 1, 'bg_color': '#EFEFEF', 'text_wrap': True
        })
        
        # Estilo dos Dias (1, 2, 3...) - Estreitos
        ...
        ws.set_column('C:C', 35)  # NOME (Largo para caber tudo)
        
        # Dias 1 a 31 (Bem estreitos, só para caber 1 número)
        # Colunas D (idx 3) até AH (idx 33)
        ws.set_column(3, 33, 2.8) 
        
        # Totais
        ws.set_column(34, 35, 6)  # DIAS e VALOR (AI, AJ)
        ...
        # Mescla título sobre todas as colunas
        ws.merge_range('A1:AJ1', titulo_texto, fmt_titulo)
        ws.merge_range('A2:AJ2', subtitulo, fmt_titulo)
        ...
        # Numeração dos dias (1 a 31)
        col_idx = 3
        for d in range(1, 32):
            if d <= u_dia:
                ws.write(2, col_idx, str(d), fmt_dia_header)
            else:
                ws.write(2, col_idx, "X", fmt_header) # Dias inexistentes (ex: 30 fev)
            col_idx += 1
        ...
        for pm in pms:
            ...
            for d in range(1, 32):
                ...
                if d <= u_dia:
                    ...
                    # Se for código numérico (0, 1, 2, 3), soma
                    if cod in ['0', '1', '2', '3']:
                        dias_trabalhados += 1
                        soma_valor += int(cod)
                        val_cell = cod
                    # Se for afastamento (A, F, LP), exibe a letra
        ...
        nome_arquivo = f"Frequencia_{cia}_{mes}_{ano}.xlsx"
